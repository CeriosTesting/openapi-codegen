...
describe("Config Loading", () => {
	const testConfigDir = join(process.cwd(), "tests", "fixtures", "config-files");

	describe("loadConfig", () => {
		it("should load JSON config file from explicit path", async () => {
			const configPath = join(testConfigDir, "openapi-to-zod.config.json");
		...
		it("should load TypeScript config file from explicit path", async () => {
			const configPath = join(testConfigDir, "openapi-to-zod.config.ts");
			const config = await loadConfig(configPath);
		...
		it("should throw error for non-existent config file", async () => {
			const configPath = join(testConfigDir, "non-existent.json");
		...
		it("should validate that at least one spec is required", async () => {
			const configPath = join(testConfigDir, "empty-specs-config.json");
