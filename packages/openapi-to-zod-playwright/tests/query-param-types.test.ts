...
describe("Query Parameter Schema Types in Service", () => {
	...
	describe("with useOperationId: false (path-based method naming)", () => {
		it("should still use operationId-based query params when operationId exists", () => {
			...
			// When useOperationId is false, the METHOD is named getUsers (from GET /users)
			// But the SCHEMA name still uses operationId because that's how the base generator names it
			// This ensures the service can find the correct schema generated by the base generator
			expect(serviceOutput).toContain("params?: SearchUsersQueryParams");
			// Should NOT use generic QueryParams type
			expect(serviceOutput).not.toContain("params?: QueryParams");
		});
		...
		it("should use the operationId-based type name even when useOperationId is false", () => {
			...
			// Schema type name should be based on operationId (to match base generator output)
			expect(serviceOutput).toContain("SearchUsersQueryParams");
		});
	});
});
